...
const { data: projects } = await useFetch('/api/portfolio/portfolio')
const { data: categories } = await useFetch('/api/portfolio/portfolioCategories')
        ...
        <section class="testimonials-modal block">
          ...
          <div class="modal-content space-y-3 mt-4">
            ...
            <small class="flex items-center justify-start gap-2 text-gray-500">
              ...
              <NuxtLink to="#">View Project</NuxtLink>
            </small>

            <p class="text-justify" v-html="locale === 'en' ? activeItem.content?.en : activeItem.content?.th" />
          </div>
        </section>
      </div>
    </section>
